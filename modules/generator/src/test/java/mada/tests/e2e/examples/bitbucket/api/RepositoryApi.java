...
public interface RepositoryApi {
    ...
    /**
     * Get recently accessed repositories. Retrieve a page of recently accessed repositories for the currently authenticated
     * user.
     * 
    ...
    /**
     * Stream archive of repository. Streams an archive of the repository's contents at the requested commit. If no `at=`
     ...
     * "Content-Disposition" header. If an explicit filename is not provided, one will be automatically generated based on
     * what is being archived. Its format depends on the at= value:
     * 
     * - No <code>at=</code> commit: &lt;slug&gt;-&lt;default-branch-name&gt;@&lt;commit&gt;.&lt;format&gt;; e.g.
     * example-master@43c2f8a0fe8.zip - <code>at=</code>sha: &lt;slug&gt;-&lt;at&gt;.&lt;format&gt;; e.g.
     * example-09bcbb00100cfbb5310fb6834a1d5ce6cac253e9.tar.gz - <code>at=</code>branchOrTag:
     * &lt;slug&gt;-&lt;branchOrTag&gt;@&lt;commit&gt;.&lt;format&gt;; e.g. example-feature@bbb225f16e1.tar
     * 
     * - If the branch or tag is qualified (e.g. refs/heads/master, the short name (master) will be included in the filename
     * - If the branch or tag's <i>short name</i> includes slashes (e.g. release/4.6), they will be converted to hyphens in
     ...
     * 
     * The <code>prefix=</code> query parameter may be used to define a directory (or multiple directories) where the
     * archive's contents should be placed. If the prefix does not end with /, one will be added automatically. The prefix
     * is <i>always</i> treated as a directory; it is not possible to use it to prepend characters to the entries in the
     * archive.
     * 
    ...
    /**
     * Get an attachment. Retrieve the attachment.
     * 
     ...
     * 
     * Range requests (see IETF RFC7233) are supported. However only a single range issupported. If multiple ranges are
     * passed the ranges will be ignored and the entire content will be returned in the response.
     *
     * @param projectKey     The project key (not null)
    ...
    /**
     * Get attachment metadata. Retrieve the attachment metadata.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the specified repository that is
    ...
    /**
     * Save attachment metadata. Save attachment metadata.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the specified repository that is
     * associated to the attachment that has the attachment metadata.
     *
    ...
    /**
     * Find branches. Retrieve the branches matching the supplied <strong>filterText</strong> param.
     * 
    ...
    /**
     * Create branch. Creates a branch using the information provided in the RestCreateBranchRequest request
     * 
    ...
    /**
     * Get file content at revision. Retrieve a page of content for a file path at a specified revision.
     * 
     * Responses from this endpoint vary widely depending on the query parameters. The example JSON is for a request that
     * does not use size, type, blame or noContent.
     * 
     * 1. size will return a response like {"size":10000} 2. type will return a response like {"type":"FILE"}, where
     * possible values are "DIRECTORY", "FILE" and "SUBMODULE" 3. blame <i>without</i> noContent will include blame for the
     * lines of content returned on the page 4. blame <i>with</i> noContent will omit file contents and only return blame
     * for the requested lines 5. noContent without blame is ignored and does nothing
     * 
     * 
     * The various parameters are "processed" in the above order. That means ?size=true&amp;type=truewill return a size
     * response, not a type one; the type parameter will be ignored.
     * 
     * The blame and noContent query parameters are handled differently from size and type. For blame and noContent, the
     * <i>presence</i> of the parameter implies "true" if no value is specified; size and and type both require an
     * explicit=true or they're treated as "false".
     * 
     * - ?blame is the same as ?blame=true - ?blame&amp;noContent is the same as ?blame=true&amp;noContent=true - ?size is
     * the same as ?size=false - ?type is the same as ?type=false
     * 
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the specified repository to call this
     * resource.
    ...
    /**
     * Get file content. Retrieve a page of content for a file path at a specified revision.
     * 
     * Responses from this endpoint vary widely depending on the query parameters. The example JSON is for a request that
     * does not use size, type, blame or noContent.
     * 
    ...
    /**
     * Edit file. Update the content of path, on the given repository and branch.
     * 
     ...
     * 
     * ```curl -X PUT -u username:password -F content=@README.md -F 'message=Updated using file-edit REST API' -F
     * branch=master -F sourceCommitId=5636641a50b
     * http://example.com/rest/api/latest/projects/PROJECT_1/repos/repo_1/browse/README.md ```
     * 
     * - branch: the branch on which the path should be modified or created - content: the full content of the file at path
     * - message: the message associated with this change, to be used as the commit message. Or null if the default message
     * should be used. - sourceCommitId: the commit ID of the file before it was edited, used to identify if content has
     * changed. Or null if this is a new file
     * 
     * 
     * The file can be updated or created on a new branch. In this case, the sourceBranch parameter should be provided to
     * identify the starting point for the new branch and the branch parameter identifies the branch to create the new
     * commit on.
     *
    ...
    /**
     * Get changes made in commit. Retrieve a page of changes made in a specified commit.
     * 
     * <strong>Note:</strong> The implementation will apply a hard cap ({@code page.max.changes}) and it is not possible to
     * request subsequent content when that cap is exceeded.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Get commits. Retrieve a page of commits from a given starting commit or "between" two commits. If no explicit commit
     * is specified, the tip of the repository's default branch is assumed. commits may be identified by branch or tag name
     * or by ID. A path may be supplied to restrict the returned commits to only those which affect that path.
     * 
    ...
    /**
     * Get commit by ID. Retrieve a single commit <i>identified by its ID</i>. In general, that ID is a SHA1. <u>From 2.11,
    ...
    /**
     * Get changes in commit. Retrieve a page of changes made in a specified commit.
     * 
     * <strong>Note:</strong> The implementation will apply a hard cap (<code>page.max.changes</code>) and it is not
    ...
    /**
     * Search for commit comments. Retrieves the commit discussion comments that match the specified search criteria.
     * 
     * It is possible to retrieve commit discussion comments that are anchored to a range of commits by providing the
     * sinceId that the comments anchored from.
     * 
    ...
    /**
     * Add a new commit comment. Add a new comment.
     * 
     ...
     * "path/to/file", "srcPath": "path/to/file", "toHash": "04c7c5c931b9418ca7b66f51fe934d0bd9b2ba4b" } } ```
     * 
     ...
     * 'srcPath' refers to the path the that file used to have (only required for copies and moves). Also, fromHash and
     * toHash refer to the sinceId / untilId (respectively) used to produce the diff on which the comment was added. Finally
     * diffType refers to the type of diff the comment was added on.
     * 
     * For line comments, 'line' refers to the line in the diff that the comment should apply to. 'lineType' refers to the
     * type of diff hunk, which can be:- 'ADDED' - for an added line;</li>- 'REMOVED' - for a removed line; or</li>-
     * 'CONTEXT' - for a line that was unmodified but is in the vicinity of the diff.</li>'fileType' refers to the file of
     * the diff to which the anchor should be attached - which is of relevance when displaying the diff in a side-by-side
     * way. Currently the supported values are:- 'FROM' - the source file of the diff</li>- 'TO' - the destination file of
     * the diff</li>If the current user is not a participant the user is added as one and updated to watch the commit.
     * 
    ...
    /**
     * Get a commit comment. Retrieves a commit discussion comment.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the repository that the commit is in to
     * call this resource.
     *
    ...
    /**
     * Update a commit comment. Update a comment, with the following restrictions:
     * 
     * - only the author of the comment may update the <i>text</i> of the comment - only the author of the comment or
     * repository admins and above may update the other fields of a comment
     * 
     * 
    ...
    /**
     * Delete a commit comment. Delete a commit comment. Anyone can delete their own comment. Only users with
     * <strong>REPO_ADMIN</strong> and above may delete comments created by other users. Comments which have replies <i>may
     * not be deleted</i>, regardless of the user's granted permissions.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the repository that the commit is in to
     * call this resource.
     *
    ...
    /**
     * Get diff between revisions. Retrieve the diff between two provided revisions.
     * 
     * To stream a raw text representation of the diff, this endpoint can be called with the request header 'Accept:
     * text/plain'.
     * 
     * Note:</strong> This resource is currently <i>not paged</i>. The server will internally apply a hard cap to the
     * streamed lines, and it is not possible to request subsequent pages if that cap is exceeded. In the event that the cap
     * is reached, the diff will be cut short and one or more {@code truncated} flags will be set to true on the "segments",
    ...
    /**
     * Get the common ancestor between two commits. Returns the best common ancestor between two commits.
     * 
    ...
    /**
     * Watch commit. Add the authenticated user as a watcher for the specified commit.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the repository containing the commit to
     * call this resource.
     *
     * @param projectKey     The project key (not null)
     * @param commitId       The <i>full ID</i> of the commit within the repository (not null)
    ...
    /**
     * Stop watching commit. Remove the authenticated user as a watcher for the specified commit.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the repository containing the commit to
     * call this resource.
     *
    ...
    /**
     * Compare commits. Gets the file changes available in the <code> from</code> commit but not in the <code> to</code>
     * commit.
     * 
     * 
     * If either the <code> from</code> or <code> to</code> commit are not specified, they will be replaced by the default
    ...
    /**
     * Get accessible commits. Gets the commits accessible from the <code>from</code> commit but not in the <code>to</code>
     * commit.
     * 
     * If either the <code>from</code> or <code>to</code> commit are not specified, they will be replaced by the default
     * branch of their containing repository.
    ...
    /**
     * Get diff between commits. Gets a diff of the changes available in the <code>from</code> commit but not in the
     * <code> to</code> commit.
     * 
     * If either the <code> from</code> or <code> to</code> commit are not specified, they will be replaced by the default
     * branch of their containing repository.
    ...
    /**
     * Get raw diff for path. Stream the raw diff between two provided revisions.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Get files in directory. Retrieve a page of files from particular directory of a repository. The search is done
    ...
    /**
     * Get hook scripts. Return a page of hook scripts configured for the specified repository.
     * 
    ...
    /**
     * Create/update a hook script. Creates/updates the hook script configuration for the provided hook script and
     * repository.
     * 
     * This endpoint requires **REPO_ADMIN** permission.
     *
    ...
    /**
     * Remove a hook script. Removes the hook script from the set of hook scripts configured to run in the repository.
     * 
     * This endpoint requires **REPO_ADMIN** permission.
     *
     * @param projectKey     The project key. (not null)
    ...
    /**
     * Get repository labels. Get all labels applied to the given repository.
     * 
    ...
    /**
     * Add repository label. Applies a label to the repository.
     * 
    ...
    /**
     * Remove repository label. Remove label that is applied to the given repository.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository.
     *
     * @param projectKey     The project key. (not null)
    ...
    /**
     * Stream files. Streams files from the repository's root with the last commit to modify each file. Commit modifications
    ...
    /**
     * Stream files with last modified commit in path. Streams files in the requested <code>path</code> with the last commit
     * to modify each file. Commit modifications are traversed starting from the <code>at</code> commit or, if not
     * specified, from the tip of the default branch.
     * 
     * Unless the repository is public, the authenticated user must have <b>REPO_READ</b> access to call this resource.
     *
    ...
    /**
     * Get patch content at revision. Retrieve the patch content for a repository at a specified revision.
     * 
    ...
    /**
     * Get raw content of a file at revision. Retrieve the raw content for a file path at a specified revision.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Get ref change activity. Retrieve a page of repository ref change activity.
     * 
    ...
    /**
     * Get branches with ref change activities for repository. Retrieve a page of branches with ref change activities for a
     * specific repository.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission to call this resource.
     *
    ...
    /**
     * Get auto decline settings. Retrieves the auto decline settings for the supplied repository. Project settings will be
     * returned if no explicit settings have been set for the repository. In the case that there are no project settings,
    ...
    /**
     * Create auto decline settings. Creates or updates the auto decline settings for the supplied repository.
     * 
    ...
    /**
     * Delete auto decline settings. Delete auto decline settings for the supplied repository.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for this repository to call the resource.
     *
     * @param projectKey     The project key (not null)
    ...
    /**
     * Get pull request auto-merge settings. Retrieves the pull request auto-merge settings for the supplied repository.
     * Project settings will be returned if no explicit settings have been set for the repository. In the case that there
     * are no project settings, the default settings will be returned. If the repository's project has restricted its
     * auto-merge settings, then the settings of the project will be returned.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for this repository to call the resource.
     *
     * @param projectKey     The project key (not null)
    ...
    /**
     * Create or update the pull request auto-merge settings. Creates or updates the pull request auto-merge settings for
    ...
    /**
     * Delete pull request auto-merge settings. Deletes pull request auto-merge settings for the supplied repository.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for this repository to call the resource.
     *
     * @param projectKey     The project key (not null)
    ...
    /**
     * Get repository hooks. Retrieve a page of repository hooks for this repository.
     * 
    ...
    /**
     * Get repository hook. Retrieve a repository hook for this repository.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Delete repository hook. Delete repository hook configuration for the supplied <strong>hookKey</strong> and
    ...
    /**
     * Enable repository hook. Enable a repository hook for this repository and optionally apply new configuration.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository to call this
     * resource.
     * 
     * A JSON document may be provided to use as the settings for the hook. These structure and validity of the document is
     * decided by the plugin providing the hook.
     *
     * @param projectKey     The project key. (not null)
     * @param hookKey        The hook key. (not null)
    ...
    /**
     * Disable repository hook. Disable a repository hook for this repository.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Get repository hook settings. Retrieve the settings for a repository hook for this repository.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Update repository hook settings. Modify the settings for a repository hook for this repository.
     * 
     ...
     * resource.
     * 
     * A JSON document can be provided to use as the settings for the hook. These structure and validity of the document is
     * decided by the plugin providing the hook.
     *
     * @param projectKey     The project key. (not null)
     * @param hookKey        The hook key. (not null)
    ...
    /**
     * Get pull request settings. Retrieve the pull request settings for the context repository.
     * 
     ...
     * 
     * - mergeConfig - the merge strategy configuration for pull requests - requiredApprovers - (Deprecated, please use
    ...
    /**
     * Update pull request settings. Update the pull request settings for the context repository.
     * 
     ...
     * - mergeConfig - the merge strategy configuration for pull requests - requiredApprovers - (Deprecated, please use
     * com.atlassian.bitbucket.server.bundled-hooks.requiredApproversMergeHook instead) the number of approvals required on
     * a pull request for it to be mergeable, or 0 to disable the merge check -
     ...
     * 
     * - The configuration set for this repository's SCM type as set at the project level, if present, otherwise - the
     * configuration set for this repository's SCM type as set at the instance level, if present, otherwise - the default
     * configuration for this repository's SCM type
     *
     * 
     * 
     * 
    ...
    /**
     * Find tag. Retrieve the tags matching the supplied <strong>filterText</strong> param.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the context repository to call this
     * resource.
     *
    ...
    /**
     * Create tag. Creates a tag using the information provided in the RestCreateTagRequest request
     * 
     * The authenticated user must have <strong>REPO_WRITE</strong> permission for the context repository to call this
     * resource.
     *
    ...
    /**
     * Get tag. Retrieve a tag in the specified repository.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the context repository to call this
     * resource.
     *
    ...
    /**
     * Watch repository. Add the authenticated user as a watcher for the specified repository.
     * 
    ...
    /**
     * Stop watching repository. Remove the authenticated user as a watcher for the specified repository.
     * 
     * The authenticated user must have <strong>REPO_READ</strong> permission for the repository to call this resource.
     *
     * @param projectKey     The project key. (not null)
    ...
    /**
     * Find webhooks. Find webhooks in this repository.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Create webhook. Create a webhook for the repository specified via the URL.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Search webhooks. Search webhooks in this repository and parent project. This endpoint returns a superset of the
     * results returned by the /webhooks endpoint because it allows filtering by project scope too, not just repository
     * webhooks.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository to call this
     * resource.
    ...
    /**
     * Test webhook. Test connectivity to a specific endpoint.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Delete webhook. Delete a webhook for the repository specified via the URL.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Get last webhook invocation details. Get the latest invocations for a specific webhook.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Get webhook statistics. Get the statistics for a specific webhook.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Get webhook statistics summary. Get the statistics summary for a specific webhook.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission for the specified repository to call this
     * resource.
     *
    ...
    /**
     * Search for repositories. Retrieve a page of repositories based on query parameters that control the search. See the
     ...
     * is assumed. Please note that this permission is lower than the <tt>REPO_READ</tt> permission rather than being equal
     * to it. The implicit 'read' permission for a given repository is assigned to any user that has any of the higher
     * permissions, such as <tt>REPO_READ</tt>, as well as to anonymous users if the repository is marked as public. The
     * important implication of the above is that an anonymous request to this resource with a permission level
     * <tt>REPO_READ</tt> is guaranteed to receive an empty list of repositories as a result. For anonymous requests it is
     * therefore recommended to not specify the <tt>permission</tt> parameter at all.
     *
    ...
    /**
     * Search for ref restrictions. Search for restrictions using the supplied parameters.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission or higher to call this resource. Only
    ...
    /**
     * Create multiple ref restrictions. Allows creating multiple restrictions at once.
     *
     * @param projectKey     The project key. (not null)
     * @param repositorySlug The repository slug. (not null)
    ...
    /**
     * Get a ref restriction. Returns a restriction as specified by a restriction id.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission or higher to call this resource. Only
     * authenticated users may call this resource.
     *
    ...
    /**
     * Delete a ref restriction. Deletes a restriction as specified by a restriction id.
     * 
     * The authenticated user must have <strong>REPO_ADMIN</strong> permission or higher to call this resource. Only
     * authenticated users may call this resource.
     *
     ...
     * 
     * 
     * The authenticated user must have an effective <strong>REPO_WRITE</strong> permission to call this resource. If branch
    ...
    /**
     * Delete branch. Deletes a branch in the specified repository.
     * 
     * 
     * If the branch does not exist, this operation will not raise an error. In other words after calling this resource and
     * receiving a 204 response the branch provided in the request is guaranteed to not exist in the specified repository
     * any more, regardless of its existence beforehand.
     * 
     * 
     * The optional 'endPoint' parameter of the request may contain a commit ID that the provided ref name is expected to
     * point to. Should the ref point to a different commit ID, a 400 response will be returned with appropriate error
     * details.
     * 
     * 
     * The authenticated user must have an effective <strong>REPO_WRITE</strong> permission to call this resource. If branch
    ...
    /**
     * React to a comment. Add an emoticon reaction to a comment
     *
     * @param projectKey     The project key. (not null)
    ...
    /**
     * Remove a reaction from comment. Remove an emoticon reaction from a comment
     *
     * @param projectKey     The project key. (not null)
     * @param commentId      The comment id (not null)
     * @param commitId       The commit id (not null)
    ...
    /**
     * Create tag. Creates a tag in the specified repository.
     * 
     * The authenticated user must have an effective <strong>REPO_WRITE</strong> permission to call this resource.
     * 
     * 'LIGHTWEIGHT' and 'ANNOTATED' are the two type of tags that can be created. The 'startPoint' can either be a ref or a
     * 'commit'.
     *
     * @param projectKey     The project key. (not null)
     * @param repositorySlug The repository slug. (not null)
    ...
    /**
     * Delete tag. Deletes a tag in the specified repository.
     * 
     * The authenticated user must have an effective <strong>REPO_WRITE</strong> permission to call this resource.
     *
     * @param projectKey     The project key. (not null)
    ...
    /**
     * Get synchronization status. Retrieves the synchronization status for the specified repository. In addition to listing
     * refs which cannot be synchronized, if any, the status also provides the timestamp for the most recent synchronization
     * and indicates whether synchronization is available and enabled. If "?at" is specified in the URL, the synchronization
     * status for the specified ref is returned, rather than the complete repository status.
     * 
     * The authenticated user must have <b>REPO_READ</b> permission for the repository, or it must be public if the request
     * is anonymous. Additionally, after synchronization is enabled for a repository, meaning synchronization was available
     * at that time, permission changes and other actions can cause it to become unavailable. Even when synchronization is
     * enabled, if it is no longer available for the repository it will not be performed.
     *
     * @param projectKey     The project key. (not null)
    ...
    /**
     * Disable synchronization. Enables or disables synchronization for the specified repository. When synchronization is
     * enabled, branches within the repository are immediately synchronized and the status is updated with the outcome. That
     * initial synchronization is performed before the REST request returns, allowing it to return the updated status.
     * 
     * The authenticated user must have <b>REPO_ADMIN</b> permission for the specified repository. Anonymous users cannot
     * manage synchronization, even on public repositories. Additionally, synchronization must be available for the
     * specified repository. Synchronization is only available if:
     * 
     * - The repository is a fork, since its origin is used as upstream - The owning user still has access to the fork's
     * origin, if the repository is a <i>personalfork</i>
     *
     * @param projectKey     The project key. (not null)
     * @param repositorySlug The repository slug. (not null)
     * @param dto            (optional)
    ...
    /**
     * Manual synchronization. Allows developers to apply a manual operation to bring a ref back in sync with upstream when
     * it becomes out of sync due to conflicting changes. The following actions are supported:
     * 
     * - <tt>MERGE</tt>: Merges in commits from the upstream ref. After applying this action, the synchronized ref will be
     * <tt>AHEAD</tt> (as it still includes commits that do not exist upstream. - This action is only supported for
     * <tt>DIVERGED</tt> refs - If a "commitMessage" is provided in the context, it will be used on the merge commit.
     * Otherwise a default message is used. - <tt>DISCARD</tt>: <i>Throws away</i> local changes in favour of those made
     * upstream. This is a <i>destructive</i> operation where commits in the local repository are lost. - No context entries
     * are supported for this action - If the upstream ref has been deleted, the local ref is deleted as well - Otherwise,
     * the local ref is updated to reference the same commit as upstream, even if the update is not fast-forward (similar to
     * a forced push)
     * 
     * 
     * The authenticated user must have <b>REPO_WRITE</b> permission for the specified repository. Anonymous users cannot
     * synchronize refs, even on public repositories. Additionally, synchronization must be <i>enabled</i> and
     * <i>available</i> for the specified repository.
     *
     * @param projectKey     The project key. (not null)
     * @param repositorySlug The repository slug. (not null)
     * @param dto            (optional)
