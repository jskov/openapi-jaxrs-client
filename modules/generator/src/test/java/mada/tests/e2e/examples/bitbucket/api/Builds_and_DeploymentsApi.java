...
public interface Builds_and_DeploymentsApi {
     ...
     *
     * Get a specific build status. The authenticated user must have **REPO_READ** permission for the provided
     * repository.The request can also be made with anonymous 2-legged OAuth.<br>
     * Since 7.14
     *
     ...
     *
     * Store a build status. The authenticated user must have **REPO_READ** permission for the repository that this build
     ...
     *
     * Delete a specific build status. The authenticated user must have **REPO_ADMIN** permission for the provided
     * repository.
     *
     * @param projectKey     The project key. (not null)
     * @param commitId       The commit. (not null)
     ...
     * Get a deployment.
     *
     * Get the deployment matching the specified Repository, key, environmentKey and deploymentSequenceNumber. The user must
     * have REPO_READ.
     *
     * @param projectKey               The project key (not null)
     ...
     * Delete a deployment.
     *
     * Delete the deployment matching the specified Repository, key, environmentKey and deploymentSequenceNumber. The user
     * must have REPO_ADMIN.
     *
     * @param projectKey               The project key (not null)
     ...
     * them</em> will not be returned.<br>
     ...
     *
     * Create a new insight report, or replace the existing one if a report already exists for the given repository, commit,
     * and report key. A request to replace an existing report will be rejected if the authenticated user was not the
     * creator of the specified report. The report key should be a unique string chosen by the reporter and should be unique
     * enough not to potentially clash with report keys from other reporters. We recommend using reverse DNS namespacing or
     * a similar standard to ensure that collision is avoided.
     ...
     *
     * Add annotations to the given report. The request should be a JSON object mapping the string "annotations" to an array
     * of maps containing the annotation data, as described below. See also the example request. A few things to note:-
     * Annotations are an extension of a report, so a report must first exist in order to post annotations. Annotations are
     * posted separately from the report, and can be posted in bulk using this endpoint. - Only the annotations that are on
     * lines changed in the unified diff will be displayed. This means it is likely not all annotations posted will be
     * displayed on the pull request It also means that if the user is viewing a side-by-side diff, commit diff or iterative
     * review diff they will not be able to view the annotations. - A report cannot have more than 1000 annotations by
     * default, however this property is congurable at an instance level. If the request would result in more than the
     * maximum number of annotations being stored then the entire request is rejected and no new annotations are stored. -
     * There is no de-duplication of annotations on Bitbucket so be sure that reruns of builds will first delete the report
     * and annotations before creating them. # Annotation parameters |Parameter|Description|Required?|Restrictions|Type|
     * |--- |--- |--- |--- |--- | |path|The path of the file on which this annotation should be placed. This is the path of
     * the filerelative to the git repository. If no path is provided, then it will appear in the overview modalon all pull
     * requests where the tip of the branch is the given commit, regardless of which files weremodified.|No||String|
     * |line|The line number that the annotation should belong to. If no line number is provided, then it willdefault to 0
     * and in a pull request it will appear at the top of the file specified by the path field.|No|Non-negative
     * integer|Integer| |message|The message to display to users|Yes|The maximum length accepted is 2000 characters, however
     * the user interface may truncate this valuefor display purposes. We recommend that the message is short and succinct,
     * with further detailsavailable to the user if needed on the page linked to by the the annotation link.|String|
     * |severity|The severity of the annotation|Yes|One of: LOW, MEDIUM, HIGH|String| |link|An http or https URL
     * representing the location of the annotation in the external tool|No||String| |type|The type of annotation
     * posted|No|One of: VULNERABILITY, CODE_SMELL, BUG|String| |externalId|If the caller requires a link to get or modify
     * this annotation, then an ID must be provided. It isnot used or required by Bitbucket, but only by the annotation
     * creator for updating or deleting thisspecific annotation.|No|A string value shorter than 450 characters|String|
     *
     * @param projectKey     The project key. (not null)
     * @param commitId       The commit ID on which to record the annotation. This must be a full 40 character commit hash.
     *                       (not null)
     ...
     *
     * Delete annotations for a given report that match the given external IDs, or all annotations if no external IDs are
     * provided.
     *
     * @param projectKey     The project key. (not null)
     ...
     *
     * Create an annotation with the given external ID, or replace it if it already exists. A request to replace an existing
     * annotation will be rejected if the authenticated user was not the creator of the specified report.
     *
     * @param projectKey     The project key. (not null)
     ...
     *
     * Create a required build merge check for the given repository. The authenticated user must have **REPO_ADMIN**
     * permission for the target repository to register a required build merge check. The contents of the required build
     * merge check request are: These fields are **required**: - **buildParentKeys**: A non-empty list of build parent keys
     * that require green builds for this merge check to pass - **refMatcher.id**: The value to match refs against in the
     * target branch - **refMatcher.type.id**: The type of ref matcher, one of: "ANY_REF", "BRANCH", "PATTERN",
     * "MODEL_CATEGORY" or "MODEL_BRANCH" These fields are optional: - **exemptRefMatcher.id** The value to exempt refs in
     * the source branch from this check - **exemptRefMatcher.type.id**: The type of exempt ref matcher, one of: "ANY_REF",
     ...
     *
     * Update the required builds merge check for the given ID. The authenticated user must have **REPO_ADMIN** permission
     * for the target repository to update a required build merge check. The contents of the required build merge check
     * request are: These fields are **required**: - **buildParentKeys**: A non-empty list of build parent keys that require
     * green builds for this merge check to pass - **refMatcher.id**: The value to match refs against in the target branch -
     * **refMatcher.type.id**: The type of ref matcher, one of: "ANY_REF", "BRANCH", "PATTERN", "MODEL_CATEGORY" or
     * "MODEL_BRANCH" These fields are optional: - **exemptRefMatcher.id** The value to exempt refs in the source branch
     * from this check - **exemptRefMatcher.type.id**: The type of exempt ref matcher, one of: "ANY_REF", "BRANCH",
     ...
     *
     * Deletes a required build existing merge check, given it's ID. The authenticated user must have **REPO_ADMIN**
     * permission for the target repository to delete a required build merge check.
     *
     * @param projectKey     The project that the repository belongs to (not null)
     * @param id             (not null)
     * @param repositorySlug The repository being used (not null)
     ...
     *
     * Returns a page of required build merge checks that have been configured for this repository. The authenticated user
     * must have **REPO_READ** permission for the target repository to request a page of required build merge checks.
     *
     * @param projectKey     The project that the repository belongs to (not null)
     * @param repositorySlug The repository being used (not null)
     * @param start          Start number for the page (inclusive). If not passed, first page is assumed. (optional)
